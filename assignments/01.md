...
3. Now, create a consumer for the *Purchase* context.
    ...
    - Copy the `consumer` service definition in `docker-compose.yml`. Pick a different service name, and make sure it points to `src/Purchase/consumer.php` (instead of the one in `Common`).
